...
public class ExcelDetectionTypeController {
    ...
    public void writeExcelFile(@PathVariable(value = "rowCount") Integer rowCount, HttpServletResponse response, Principal principal) {
        ...
        cell.setCellValue(lang.equals("AR") ? "وحدة القياس" : "Unit");
        cell.setCellType(CellType.STRING);
        cell.setCellStyle(styleColumnHeader);
        ...
        CellRangeAddressList addressList = new CellRangeAddressList(1, rowCount, 6, 6);
        XSSFDataValidation validation = (XSSFDataValidation) dvHelper.createValidation(dvConstraint, addressList);
        validation.setShowErrorBox(true);
        validation.setErrorStyle(DataValidation.ErrorStyle.STOP);
        validation.createErrorBox(lang.equals("AR") ? "مركز السلطان للصقور" : "SULTAN CENTER", lang.equals("AR") ? "فضلاً اختر الحالة دون تعديل من القائمة." : "Please Select Only Options From List.");
                ...
                while (cellIterator.hasNext()) {
                    ...
                    switch (columnIndex) {
                        ...
                        case 5:
                            ...
                            Double unitCost;
                            try {
                                unitCost = Double.parseDouble((String) excelCellHelper.getCellValue(nextCell));
                            } catch (Exception ex) {
                                accept = false;
                                break;
                            }
                            detectionType.setUnitCost(unitCost);
                            log.info((String) excelCellHelper.getCellValue(nextCell));
                            break;
                        case 6:
                            if (excelCellHelper.getCellValue(nextCell) == null) {
                            ...
                            nextCell.setCellType(CellType.STRING);
                            detectionType.setUnit(lang.equals("AR") ? DrugUnit.findByName((String) excelCellHelper.getCellValue(nextCell)) : DrugUnit.findByValue((String) excelCellHelper.getCellValue(nextCell)));
                            log.info((String) excelCellHelper.getCellValue(nextCell));
                            break;
                    }
